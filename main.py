...
from contactos import agregar_contacto, eliminar_contacto, mostrar_info_contacto, limpiar_lista
...
menu_principal.add_command(label="Limpiar lista", command=lambda: limpiar_lista(lista_contactos, contactos))
menu_principal.add_command(label="Descargar contactos", command=lambda: exportar_contactos(contactos))
...
boton_agregar = tk.Button(ventana, text="Agregar Contacto", 
                          command=lambda: agregar_contacto(entrada_nombre, entrada_apellido,
                                                            entrada_telefono, entrada_direccion,
...
boton_eliminar = tk.Button(
    marco_botones_abajo, text="Eliminar Contacto", command=lambda: eliminar_contacto(lista_contactos, contactos)
)
...
boton_mostrar_info = tk.Button(
    marco_botones_abajo, text="Mostrar Info", command=lambda: mostrar_info_contacto(lista_contactos, contactos, ventana)
)
boton_mostrar_info.pack(side="left", anchor="nw", padx=(0, 10))

# Boton para agregar dummy info
boton_fake = tk.Button(
    marco_botones_abajo, text="F data", command=lambda: agregar_contactos_falsos(lista_contactos, contactos)
)
